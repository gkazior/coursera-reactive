...
object NotesWs {
  ...
  val myRandomInt = integers.generate             //> myRandomInt  : Int = 1826081464
  val myRandomBoolean = booleans.generate         //> myRandomBoolean  : Boolean = true
  val oneOfThem = oneOf(1, 2, 3, 4, 5) generate   //> oneOfThem  : Int = 4

  val myList = lists generate                     //> myList  : List[Int] = List()

  val res = List(1, 2, 3, 4) map ((x: Int) => 2 * x)
